...
tags: [集合,数据结构]
author: ligson
description: HashMap是基于哈希表的Map接口的非同步实现。此实现提供所有可选的映射操作，并允许使用null值和null键。此类不保证映射的顺序，特别是它不保证该顺序恒久不变。
---
...
![hashmap1]({{site.baseurl}}/assets/postimg/hashmap1.jpg)
...
补充一点内容：HashMap中的是通过`transient Entry<K,V>[] table`来存储数据，该变量是通过transient进行修饰的，关于对transient 在集合中的理解，可以参考这篇文章Java集合学习9：对集合中transient的理解
